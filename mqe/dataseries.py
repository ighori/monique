...
class SeriesSpec(object):
    ...
    def get_materializer(self):
        materializer_type = self.params.get('materializer_type')
        if not materializer_type:
            return materializers.DefaultMaterializer()
        cls = getattr(materializers, materializer_type)
        assert issubclass(cls, materializers.Materializer)
        kwargs = {}
        return cls(**kwargs)
